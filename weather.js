        ...
        async function getWeather() {
            try {
                ...
                if (data.current && data.daily && data.hourly) {
                    ...
                    const now = new Date();
                    const dateStr = now.toLocaleDateString('ru-RU', { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                    ...
                    setWeatherIcon(document.querySelector('.current-weather-icon'), current.weather_code, isCurrentDay);
                    setWeatherIcon(document.querySelector('.summary-icon'), current.weather_code, isCurrentDay);

                    // ДОП. ПОКАЗАТЕЛИ
                    setTextContent('[data-pressure]', formatValue(current.pressure_msl, ' гПа'));
                    setTextContent('[data-uv]', formatValue(current.uv_index));
                    setTextContent('[data-wind-direction]', getWindDirectionLabel(current.wind_direction_10m));
